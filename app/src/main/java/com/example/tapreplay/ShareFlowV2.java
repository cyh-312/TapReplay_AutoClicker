...
/**
 * V0.4 分享流程。
 * 重点修正：绝不把“分享此刻想法/分享你的想法”等底部输入入口当成分享按钮。
 */
public final class ShareFlowV2 {
    private ShareFlowV2() {}
    ...
    public static boolean shareToTarget(
        ...
        TapAccessibilityService.setOverlayStatus("符合｜正在打开分享栏…");

        if (!openRealSharePanel(service, running)) {
            TapAccessibilityService.setOverlayStatus("没打开分享栏｜已停下");
            return false;
        }
        ...
        for (int page = 0; page < 7 && running.get(); page++) {
            ...
            if (!isSharePanelOpen(service, root)) {
                root.recycle();
                TapAccessibilityService.setOverlayStatus("分享栏不见了｜已停下");
                return false;
            }

            List<AccessibilityNodeInfo> matches = findTargetContacts(service, root, target);
            root.recycle();
    ...
    private static boolean openRealSharePanel(
            TapAccessibilityService service,
        ...
        for (int round = 1; round <= 4 && running.get(); round++) {
            ...
            List<NodeScore> candidates = findRealShareCandidates(service, root);
            root.recycle();

            if (candidates.isEmpty()) {
                TapAccessibilityService.setOverlayStatus("没看到右侧分享按钮｜再找一次 " + round + "/4");
                SystemClock.sleep(260);
                continue;
            }

            // 最多尝试两个高可信候选，每次点击后都必须真正看到分享栏才算成功。
            int count = Math.min(2, candidates.size());
            for (int i = 0; i < count && running.get(); i++) {
                Rect b = candidates.get(i).bounds;
                TapAccessibilityService.setOverlayStatus("正在点右侧分享按钮｜" + round + "/4");
                tap(service, b.centerX(), b.centerY(), 70);

                if (waitSharePanel(service, 1500, running)) {
                    recycleCandidates(candidates);
                    return true;
                }

                // 如果误弹键盘，立即收起，但不会把这次误点当成分享成功。
                dismissKeyboardIfVisible(service, running);
                SystemClock.sleep(180);
            }
    ...
    private static List<NodeScore> findRealShareCandidates(
            TapAccessibilityService service,
        ...
        while (!q.isEmpty()) {
            ...
            // 只接受“分享”本身，或类似“分享11.1万，按钮”的无障碍描述。
            // 不再接受任何“以分享开头的短文字”，从根上排除“分享此刻想法”。
            boolean strongShareLabel =
                    "分享".equals(tx) ||
                    "分享".equals(ds) ||
                    (ds.startsWith("分享") && ds.contains("按钮"));

            boolean thoughtComposer =
                    tx.contains("分享此刻想法") || ds.contains("分享此刻想法") ||
                    tx.contains("分享你的想法") || ds.contains("分享你的想法") ||
                    tx.contains("说点什么") || ds.contains("说点什么") ||
                    tx.contains("评论") && cls.contains("edittext") ||
            ...
            // 真正的分享按钮在视频右侧操作栏。底部输入框即使父节点很宽，也不会进这里。
            boolean inRightRail =
                    b.centerX() >= w * 0.72f &&
                    b.centerY() >= h * 0.12f &&
                    b.centerY() <= h * 0.885f;
            ...
            if (strongShareLabel && !forbidden && inRightRail && sensibleSize) {
                addCandidate(out, n, b, scoreShareNode(n, tx, ds, b, w));

                // 仅从“高可信分享文字/描述”向上找最多2层可点击容器。
                ...
                for (int depth = 0; p != null && depth < 2; depth++) {
                    ...
                    if (p.isVisibleToUser() && p.isEnabled() &&
                            ...
                            pb.centerX() >= w * 0.72f &&
                            pb.centerY() >= h * 0.12f && pb.centerY() <= h * 0.885f &&
                            pb.width() <= w * 0.28f && pb.height() <= h * 0.16f) {
                        int score = scoreShareNode(p, tx, ds, pb, w) +
                                (p.isClickable() ? 90 : 15) - depth * 10;
    ...
    private static int scoreShareNode(
        ...
        int score = 0;
        if (ds.startsWith("分享") && ds.contains("按钮")) score += 260;
        if ("分享".equals(tx) || "分享".equals(ds)) score += 150;
        if (b.centerX() >= w * 0.82f) score += 70;
        if (n.isClickable()) score += 45;
        if (b.width() >= 30 && b.height() >= 30) score += 15;
        return score;
    }
    ...
    private static void addCandidate(
        ...
        for (NodeScore e : out) {
            if (Math.abs(e.bounds.centerX() - bounds.centerX()) <= 10 &&
                    ...
                    Math.abs(e.bounds.width() - bounds.width()) <= 14 &&
                    Math.abs(e.bounds.height() - bounds.height()) <= 14) {
                if (score > e.score) e.score = score;
                return;
            }
        }
        out.add(new NodeScore(AccessibilityNodeInfo.obtain(node), new Rect(bounds), score));
    }
    ...
    private static boolean isSharePanelOpen(
        ...
        return exactTitle || (cancel && lowerScrollable) || actions.size() >= 2;
    }
    ...
    private static List<AccessibilityNodeInfo> findTargetContacts(
        ...
        while (!q.isEmpty()) {
            ...
            if (exact && plausibleY && !cls.contains("edittext") && n.isVisibleToUser()) {
                ...
                if (clickable != null) {
                    addUniqueNode(found, clickable);
                    clickable.recycle();
                } else {
                    addUniqueNode(found, AccessibilityNodeInfo.obtain(n));
                }
            }
