...
public class ModelEngine {
    ...
    // Internal stability-only changes. Recognition preprocessing, prompts, gates and thresholds
    // remain identical to released V0.7.5.
    private static final int MAX_SIGLIP_INPUT_FRAMES = 20;
    private static final int CLIP_INFER_BATCH = 12;

    // Reuse the two largest caller-owned native/direct input buffers instead of allocating fresh
    // DirectByteBuffers every inference cycle. Access is serialized by the existing model lock.
                ...
                for (int i = 0; i < batch; i++) {
                    ...
                    for (int ch = 0; ch < 3; ch++) {
                        for (int p : clipPixels) {
                            int v = ch == 0 ? Color.red(p) : ch == 1 ? Color.green(p) : Color.blue(p);
                            float f = (v / 255f - CLIP_MEAN[ch]) / CLIP_STD[ch];
                            sb.put(floatToHalf(f));
                        }
                    }
                }
